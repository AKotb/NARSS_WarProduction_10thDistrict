...
public class SubZoneDAOImpl implements SubZoneDAO {
	...
	public SubZone getSubZone(String azCity, String azAreaCode, String azRegionCode, String azZoneCode, String azSubZoneCode) {
		...
		try {
			Session session = factory.getCurrentSession();
			Query q = session.createQuery("FROM SubZone WHERE azCity = :azcity AND azAreaCode = :azareacode AND azRegionCode = :azregioncode AND azZoneCode = :azzonecode AND azSubZoneCode = :azsubzonecode");
